...
void ElGamalEncrypt_17_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void ElGamalEncrypt_17_run(uint ctx_index,Circom_CalcWit* ctx);
void MaskAnOpenCard_18_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MaskAnOpenCard_18_run(uint ctx_index,Circom_CalcWit* ctx);
...
ElGamalEncrypt_17_run,
MaskAnOpenCard_18_run };
...
uint get_total_signal_no() {return 18062;}

uint get_number_of_components() {return 1978;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 3108;}
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ctx->templateInsId2IOSignalInfo[ctx->componentMemory[mySubcomponents[cmp_index_ref]].templateId].defs[2].offset+map_index_aux[0]];
// load src
Fr_mul(&expaux[1],&lvar[3],&circuitConstants[19]); // line circom 157
...
{
uint aux_create = 3;
int aux_cmp_num = 953+ctx_index+1;
uint csoffset = mySignalStart+10144;
for (uint i = 0; i < 1; i++) {
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[0]); // line circom 45
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[2]); // line circom 45
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[0]); // line circom 45
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[0]); // line circom 60
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 3;
{
...
ctx->componentMemory[coffset].templateName = "MaskAnOpenCard";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 7;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+12;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "elgamal";
ElGamalEncrypt_17_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 18049 ;
aux_cmp_num += 1977;
}
...
PFrElement aux_dest = &signalValues[mySignalStart + 4];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 10],&signalValues[mySignalStart + 11]); // line circom 38
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
ElGamalEncrypt_17_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
...
MaskAnOpenCard_18_create(1,0,ctx,"main",0);
MaskAnOpenCard_18_run(0,ctx);
}
