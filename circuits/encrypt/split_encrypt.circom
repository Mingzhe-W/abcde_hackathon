...
include "./split_encrypt_template.circom";
include "../../node_modules/circomlib/circuits/bitify.circom";

template SplitEncryptV2(numCards) {
    var numBits = 251;
    ...
    component split_encrypt = SplitEncryptV2Template(base, numCards, numBits);
    split_encrypt.pk[0] <== pk[0];
    split_encrypt.pk[1] <== pk[1];
    for (var i = 0; i<numCards; i++) {
        split_encrypt.UX0[i] <== UX0[i];
        split_encrypt.UX1[i] <== UX1[i];
        split_encrypt.VX0[i] <== VX0[i];
        split_encrypt.VX1[i] <== VX1[i];
        split_encrypt.UDelta0[i] <== UDelta0[i];
        split_encrypt.UDelta1[i] <== UDelta1[i];
        split_encrypt.VDelta0[i] <== VDelta0[i];
        split_encrypt.VDelta1[i] <== VDelta1[i];
    }

    for(var i = 0; i < 2; i++) {
        split_encrypt.s_u[i] <== s_u[i];
        split_encrypt.s_v[i] <== s_v[i];
    }

    split_encrypt.x <== x;

    for (var i = 0; i<numCards; i++) {
        split_encrypt.R[i] <== R[i];
    }
}

component main {public [pk, UX0, UX1, VX0, VX1, s_u, s_v]}  = SplitEncryptV2(52);