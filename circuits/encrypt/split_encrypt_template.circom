...
template ShuffleEncryptV2Template(base, numCards, numBits) {
    ...
    for (var i = 0; i < 2; i++) {
        splitEncryptV1.pk[i] <== pk[i];
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.X[i] <== UX0[i];
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.X[numCards + i] <== decompress[i].y;
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.X[2*numCards+i] <== UX1[i];
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.X[3*numCards + i] <== decompress[numCards+i].y;
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.Y[i] === VX0[i];
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.Y[numCards + i] === decompress[2*numCards + i].y;
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.Y[2*numCards + i] === VX1[i];
    }
    for (var i = 0; i < numCards; i++) {
        splitEncryptV1.Y[3*numCards + i] === decompress[3*numCards + i].y;
    }
}